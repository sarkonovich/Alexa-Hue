...
You need to set up the local Sinatra server. Place ````app.rb````, ````hue_switch.rb````, ````lights.rb````, ````alexa_objects.rb````, and ````fix_schedule_syntax.rb```` in the same directory. Open up a terminal in that directory and type
